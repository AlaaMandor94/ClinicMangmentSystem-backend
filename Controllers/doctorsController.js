...
const multerFilter = (req, file, cb) => { // CHECK IF THE UPLODED FILE IS image OR NOT 
...
exports.getAllDoctors = async(req, res, next) => {
    try {
        ...
        let query = doctorModel.find(JSON.parse(queryStr))
...
exports.getDoctorById = async(req, res, next) => {
    ...
    try {
        const doctor = await doctorModel.findById(id)
            .populate({ path: 'clinic', select: ['_id', 'name', "department", "address"] })
            .populate({
                path: 'appointments',
                populate: {
                    path: 'patient',
